...
\begin{document}
	...
	\section{Lecture 2. Jan. 14 2019\\Iterated Elimination and Rationalizability}
		...
		\subsection{Iterated Elimination of Strictly Dominated Strategies(Actions)}
				...
				\begin{itemize}
					...
					\begin{equation}
						u_i(a_i; a_{-i}) = \begin{cases}
							a_i + \frac{1}{2}(a_{-i} - a{i} - 1) \tx{ if } a_i < a_{-i} \\
			...
			\begin{definition}
				For each $i \in N$, the \textbf{best-response function} of this player is a correspondence $B_i: A_{-i} \to A_i$ defined as
				\begin{equation}
			...
			\begin{definition}
				$a_i$ is a \textbf{best response} to belief $\alpha_i$ if
				\begin{equation}
			...
			\begin{example}
				...
				\begin{proof}
					...
					\end{equation}
					Therefore, for any belief $\alpha$, $D$ cannot be a best response.
				\end{proof}
			\end{example}
			\begin{figure}[h]
				\centering
			...
			\begin{lemma}[i385.3]
				In a \ul{two player} game, $a_i$ is strictly dominated if and only if it is a never response.
			\end{lemma}
			
			\begin{definition}
				\textbf{Common knowledge rationality}
			\end{definition}
			
			\red{\paragraph{NOTE} Lecture Stops Here.}
