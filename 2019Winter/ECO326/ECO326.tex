...
\begin{document}
	...
	\section{Lecture 5. Mixed Strategies}
		\begin{definition}[lec.5]
			For player $i \in N$, a \textbf{mixed strategy} $\sigma_i$ is a \ul{probability distribution} over $A_i$.
		\end{definition}
		...
		\begin{proposition}[lec.5]
			...
			\[
				U_i(\sigma_i, \sigma_{-i}) \equiv \expect{u_i(a)|(\sigma_i)} = \sum_{a_i \in A_i} \sum_{a_{-i} \in A_{-i}} u_i(a_i, a_{-i}) \sigma_i(a_i) \sigma_{-i}(a_{-i})
			\]
		\end{proposition}
		...
		\begin{notation}
			Let $A$ be a set and let $\Delta(A)$ denote the \ul{set of probability measures} on set $A$.
		\end{notation}
