		...
		\section{Inclusion-Exclusion Principle}
			...
			\subsection{Counting Surjections}
				\begin{theorem}
					...
					\begin{gather}
						N(S) = \underbrace{\binom{m}{1} (m-1)^n}_{f \tx{ omit one element}} 
						- \underbrace{\binom{m}{2} (m-2)^n}_{f \tx{ omit two elements}}
						+ \underbrace{\binom{m}{3} (m-3)^n}_{f \tx{ omit three elements}} - \dots \\
						= \sum_{k=1}^m (-1)^{k+1} \underbrace{\binom{m}{k} (m-k)^n}_{f \tx{ omit $k$ elements}} \\
