...
\begin{document}
	...
	\section{Graph Theory}
		...
		\begin{definition}
			Given $\mc{G}$ is connected\footnote{If $\mc{G}$ is disconnected, it's impossible for any of its spanning subgraph to be connected.}, a subgraph $\mc{H} \equiv (W, F)$ of $\mc{G}$ is a \textbf{spanning graph} if both $V = W$ and $\mc{H}$ is a tree.
		\end{definition}
		...
		\subsection{Planer Graphs}
			...
			\begin{theorem}[Kuratowski's Theorem]
				A graph is planar \ul{if and only if} it does not contain either $\ve{K}_5$ or $\ve{K}_{3,3}$.
			\end{theorem}
