...
\begin{document}
	...
	\subsection{Causality and Inevitability}
		...
		\begin{theorem}
			...
			Then $\{Y_t\}$ is causal with respect to $\{\varepsilon_t\}$ \ul{if and only if} all roots of the equation $\Phi(z) = 0$ are outside the unit circle. The coefficients $\Psi$ are then uniquely defined by identity
			\begin{equation}
	...
	\begin{theorem}
		...
		\begin{equation}
			\Phi(L)Y_t = \Theta(L)\varepsilon_t
		\end{equation}
		Then $\{Y_t\}$ is invertible with  respect to $\{\varepsilon_t\}$ \ul{if and only if} all roots of $\Theta(z)$ are outside the unit circle. And the coefficients of $\Pi$ are then uniquely determined by the relation
		\begin{equation}
