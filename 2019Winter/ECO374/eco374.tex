...
\begin{document}
	...
	\section{Statistics and Time Series}
		...
		\begin{definition}[3.3]
			The \textbf{partial auto-correlation function} (PACF) $\alpha(h)$ with $h \in \Z_{\geq 0}$ of a \emph{stationary} process is defined as 
			\begin{gather}
				\alpha(0) = 1 \\
	...
	\subsection{Test for Auto-correlation}
		...
		\end{equation}
	\subsection{Causality and Inevitability}
		\begin{definition}[Causality]
			An $ARMA(p, q)$ process $\{Y_t\}$ with 
			\begin{equation}
				\Phi(L)Y_t = \Theta(L)\varepsilon_t
			\end{equation}
			is called \textbf{causal with respect to} $\{\varepsilon_t\}$ if there exists a sequence $\{\psi_j\}$ with the property $\sum_j^\infty |\psi_j| < \infty$ such that 
			\begin{equation}
		...
		\begin{theorem}
			...
			Then $\{Y_t\}$ is causal with respect to $\{\varepsilon_t\}$ \ul{if and only if} all roots $\red{\Phi(z)}$ are outside the unit circle. The coefficients $\Psi$ are then uniquely defined by identity
			\begin{equation}
	...
	\begin{theorem}
		...
		\begin{equation}
			\Phi(L)Y_t = \Theta(L)\varepsilon_t
		\end{equation}
		Then $\{Y_t\}$ is invertible with  respect to $\{\varepsilon_t\}$ \ul{if and only if} all roots of $\red{\Theta(z)}$ are outside the unit circle. And the coefficients of $\Pi$ are then uniquely determined by the relation
		\begin{equation}
