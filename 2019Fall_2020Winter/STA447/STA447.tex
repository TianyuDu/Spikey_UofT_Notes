...
\begin{document}
	...
	\begin{lemma}[Vanishing Lemma]
		If $\mc{M}$ has some $k, \ell$ such that $\lim_{n \to \infty} p_{k\ell}\upn = 0$, then for all $i, j \in S$, $\lim_{n \to \infty} p_{ij} = 0$.
	\end{lemma}
	...
	\begin{corollary}
		If there exists $i, j \in S$, $\lim_{n \to \infty} p_{ij} = 0$, then $\mc{M}$ cannot have a stationary distribution.
	\end{corollary}
