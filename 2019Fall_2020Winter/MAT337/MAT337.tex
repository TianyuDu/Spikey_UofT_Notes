...
%		\end{align}
	...
	\begin{example}
		Any constructor mapping $\mc{F}: \N \to \R^\R$ creates a sequence of functions $\R \to \R$. \\
		Taylor's polynomial is one example of sequence of functions.
	\end{example}
	...
	\begin{definition}
		Let $(f_n)$ be a sequence of functions defined on $A \subseteq \R$. Then $(f_n)$ converges uniformly on $A$ to $f: A \to \R$ if 
		\begin{align}
