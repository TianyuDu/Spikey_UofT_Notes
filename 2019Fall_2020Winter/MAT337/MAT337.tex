...
%		\end{align}
	...
	\begin{theorem}[Sequential Criterion for Absence of Uniform Continuity]
		...
		\begin{enumerate}[(i)]
			...
			\item $\exists \varepsilon_0 > 0$ and two sequences $(x_n), (y_n) \subseteq A$ such that $\abs{x_n - y_n} \to 0$ but $\abs{f(x_n) - f(y_n)} > \varepsilon_0$ for every $n \in \N$.
		\end{enumerate}
	\end{theorem}

	\begin{proof}
		$f: A \to \R$ is not uniformly continuous, \\
		\ul{iff} $\exists \varepsilon > 0\ s.t.\ \forall \delta > 0\ \exists x, y \in \R\ s.t.\ \abs{x_\delta - y_\delta} < \delta \land \abs{f(x_\delta) - f(y_\delta)} > \varepsilon_0$. \\
		For each $n \in \N$, take $\delta = \frac{1}{n}$ and construct two sequences.
	\end{proof}
	
	\begin{example}
		Let $f(x) = \sin(x^{-1})$ defined on $A = (0, 1)$. \\
		Take $x_n := \frac{1}{2n + \frac{\pi}{2}}$ and $y_n = \frac{1}{2n + \frac{3\pi}{2}}$. It is evident that $f(x_n) = 1$ and $f(y_n) = -1$ for every $n \in \N$. By taking $\varepsilon_0 = 1$, the proof is complete.
	\end{example}
\end{document}
