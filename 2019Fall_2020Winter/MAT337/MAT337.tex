...
%	\end{proof}
	...
	\begin{theorem}[Criterion of Discontinuity]
...
%		Moreover, $E = f^{-1}(A) \cup f^{-1}(B)$. \\
	...
	\begin{definition}
 	...
 	\begin{proof}
		...
		Note that for both $i=0,1$, $10^n \pi y_i$ is a multiple of $\pi$, which implies $f(y_0) = \frac{(-1)^{x_n}}{2^n}$, $f(y_1) = \frac{(-1)^{x_{n+1}}}{2^n}$
 		such that
 		\begin{align}
 	...
 	\begin{definition}
 		The \textbf{radius of convergence} is defined to be
 		\begin{align}
 			R := \sup \left\{r: \sum_{n=0}^\infty a_n r^n \tx{ converges} \right\}
 		\end{align}
 		and \textbf{interval of convergence} is defined to be the set on which the series converges.
 	\end{definition}
