...
%	\end{proof}
	...
	\begin{theorem}[The Root Test]
		Let $(a_n)$ be a sequence in which $a_n \geq 0$ for every $n \in \N$, let $\ell = \limsup \sqrt{a_n}{n}$, then
		\begin{enumerate}[(i)]
	...
	\begin{proof}
		\emph{Part 1:(Idea: compare with geometric series with $r < 1$)} Suppose $\ell < 1$, pick $r \in (\ell, 1)$, and let $\varepsilon = r - \ell$.
	\end{proof}
