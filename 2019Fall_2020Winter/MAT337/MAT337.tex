...
%		\end{align}
	...
	\begin{proposition}[Equivalent Definiton]
		$E \subseteq \R$ is disconnected if and only if it can be written as the union of two \emph{nonempty disjoint open} sets.
	\end{proposition}
	...
	\begin{example}[Thomae's Function]
		...
		\end{figure}
		\textbf{Claim: } for every $a \in \R$, $\lim_{x \to a} f(x) = 0$.
		\begin{proof}
	...
	\begin{example}
		Claim: $f(x) = x^2$ is not uniformly continuous. \\
	...
	\begin{definition}
		Let $(f_n)$ be a sequence of functions defined on $A \subseteq \R$. Then $(f_n)$t \textbf{converges uniformly} on $A$ to $f: A \to \R$ if 
		\begin{align}
	...
	\begin{theorem}
		Let $(f_n)$ be a sequence of differentiable functions defined on the closed interval $[a, b]$, and assume $(f'_n)$ \ul{converges uniformly} on $[a, b]$. If there exists a point $x_0 \in [a, b]$ where $f_n(x_n)$ converges, then $(f_n)$ converges uniformly on $[a, b]$.
	\end{theorem}
	
	\begin{theorem}[Stronger Version]
		Let $(f_n)$ be a sequence of differentiable functions defined on the closed interval $[a, b]$, and assume $(f'_n)$ converges uniformly to a function $g$ on $[a, b]$. \\
		If there exists a point $x_0 \in [a, b]$ for which $f_n(x_0)$ converges, then $(f_n)$ converges uniformly, \\
		Then the limit function $f$ is differentiable and satisfies $f' = g$.
	\end{theorem}
	...
	\begin{definition}
 	...
 	\begin{proof}
 		...
 		Further, $V_\varepsilon(x) \cap K_0 \subseteq K_\lambda^c \cap K_0$, therefore, $K_\lambda^c$ is open in $K_0$. \\
