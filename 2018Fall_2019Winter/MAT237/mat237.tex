...
\begin{document}
	...
	\section{Limits, continuity, and related topics}
		\subsection{Open and Closed Sets, Boundary, Interior and Closure}
			...
			\begin{theorem}[Homework 1.1]
				The limit
				\begin{equation}
					\lim _{(x, y) \rightarrow(0,0)} \frac{|x|^{r_{1}}|y|^{r_{2}}}{|x|^{s_{1}}+|y|^{s_{2}}}
				\end{equation}
				exists \ul{if and only if}
				\begin{equation}
					\frac{r_1}{s_1}+\frac{r_2}{s_2} \red{>} 1
				\end{equation}
			\end{theorem}
		...
		\subsection{Limits and Continuity}
			...
			\begin{definition}
				...
				is defined as
				\begin{gather}
					\forall \varepsilon > 0\ \exists \delta > 0\ s.t.\ \forall \ve{x} \in S,\ 
					\underbrace{\red{0 <} \norm{\ve{x} - \ve{a}} < \delta}_{\tx{punctured neighbourhood}}
			...
			\begin{theorem}[Limit Laws of Real Valued Functions]
			...
			\subsubsection{Continuous Functions and Open Sets}
				\begin{theorem}
					...
					\end{enumerate}
					\emph{Openness and closeness are preserved under \ul{pre-image} operations of continuous functions.}
				\end{theorem}
				
			\begin{theorem}
				An \ul{arbitrary} union of open sets is open.
			\end{theorem}
			
			\begin{theorem}
				A \ul{finite} intersection of open sets is open.
			\end{theorem}
			
			\begin{theorem}
				A \ul{finite} union of closed sets is closed.
			\end{theorem}
			
			\begin{theorem}
				An \ul{arbitrary} intersection of closed sets is closed.
			\end{theorem}
		
		\subsection{Sequences and Completeness}
			\begin{definition}
		    	    A sequence $\{\vea_j\}_{j \in \Z_{++}}$ in $\R^n$ \textbf{converges to the limit $\ve{L} \in \R^n$} if 
		    	    \begin{equation}
		    ...
		    \begin{theorem}
		    		...
		    		\begin{proof}[Proof Idea.]
		    			\begin{equation}
		    				\forall j \in [n],\ |a_j - L_j| \leq \norm{\vea - \ve{L}} \leq n \cdot \max_{k \in [n]} |a_k - L_k|
		    			\end{equation}
		    		\end{proof}
		    \end{theorem}
		    
		    \begin{axiom}[the Completeness Axiom]
		    		Every \ul{bounded} and \ul{nonempty} set of \ul{real numbers} has a \emph{least upper bound} (\textbf{supremum}) and a \emph{greatest lower bound} (\textbf{infimum}).
		    \end{axiom}
...
%		    \end{remark}
		...
		\subsection{Compactness}
			...
			\subsubsection{the Extreme Value Theorem}
				...
				\begin{theorem}[the Extreme Value Theorem]
					...
					\end{equation}
					\begin{proof}
						Let $\{y_j\}_j$ be a sequence in $f(K)$, and we can find a sequence $\{\ve{z}_j\}_j$ in $K$ such that $y_j = f(\ve{z}_j)$ (by definition of image). Because $K$ is compact, there exists a subsequence of $\{\ve{z}_j\}_j$ converges to $\ve{z}^* \in K$. Since $f$ is continuous, we can conclude there a subsequence, sharing the same indices, such that $f(\ve{z}_j) \to f(\ve{z}^*)$. Obviously $f(\ve{z}^*) \in f(K)$, \emph{so $f(K)$ is compact}. \\
						Since $f(K)$ is compact, $\sup_{\vex \in K} f(\vex) \in f(K)$. By definition of image, $\exists \vex \in K$ such that $f(\vex) = \sup_{\vex \in K} f(\vex)$, \emph{supremum attainability shown}.\\
			...
			\subsubsection{Uniform Continuity}
				\begin{definition}
					...
					\begin{equation}
						\red{
						\underbrace{\forall \varepsilon > 0\ \exists \delta > 0\ s.t.\ \forall \vex}_{\tx{cts: }\forall \vex \in S,\ \varepsilon > 0,\ \exists \delta > 0}
	...
	\section{Differentiation and related topics}
		...
		\subsection{Critical Points}
			...
			\begin{definition}
				Let $f: S \to \R$ be differentiable on the open subset $S \subset \R^n$, then a point $\vea \in S$ is a \textbf{critical point} if 
				\begin{equation}
					\nabla f(\vea) = \ve{0}
				\end{equation}
			\end{definition}
			...
			\begin{corollary}
				...
				\end{enumerate}
				\emph{If none of the above holds (semi-definiteness), then we cannot determine the character of the critical point without further thought.}
			\end{corollary}
...
%			\end{lemma}
			...
			\begin{remark}Procedure of optimization with \emph{inequality constraints}:
				\begin{enumerate}[(i)]
					\item Find critical points \emph{in} the constraint set from global critical points;
	...
	\section{The Implicit and Inverse Function Theorems}
		\subsection{The Implicit Function Theorem I}
			\begin{theorem}[Implicit Function Theorem]
				Let $S \subseteq \R^{n+k}$ be an open set, and function $\veF: S \to \R^k$ be a $C^1$ function. Suppose there exists point $\textbf{a} \in \R^n, \ve{b} \in \R^k$ such that 
				\begin{equation}
					\veF(\textbf{a}, \textbf{b}) = \textbf{0}
				\end{equation}
				If 
				\begin{equation}
					det(D_{\textbf{y}}(\veF(\textbf{a}, \textbf{b}))) \neq 0
				\end{equation}
				then there exists $r_0, r_1 > 0$ and a $C^1$ function $\ve{f}: \R^n \to \R^k$ such that
				\begin{gather}
					\forall \ve{x} \in \mc{B}(r_0, \ve{a}),\ \ve{f}(\ve{x}) \in \mc{B}(r_1, \ve{b}) \land \veF(\ve{x}, \ve{f}(\ve{x})) = \ve{0}
				\end{gather}
				and define $\textbf{y} := \textbf{f}(\textbf{x})$, the derivative of $\textbf{f}$ can be found as
				\begin{equation}
					\red{D\textbf{f}(\textbf{x}) = -[D_{\textbf{y}}\veF(\textbf{x}, \textbf{y})]^{-1} D_{\textbf{x}}\veF(\textbf{x}, \textbf{y})}
				\end{equation}
			\end{theorem}
			\begin{remark}
		...
		\subsection{Geometric Content of the Implicit Function Theorem}
			...
			\begin{remark}[Interpretation]
				In $\R^n$, there are $n$ degree of freedom before imposing any constraint, after adding constraints $\veF(\vex) = \ve{0}$ with $n-k$ effective constraints, there are $k$ degree of freedom left. What's left is a $k$ dimensional manifold.
			\end{remark}
	...
	\section{Integration}
		\subsection{Basics}
			\begin{theorem}[\red{Properties of infimum and supremum}]
				Let $A \subseteq \R^n$ and $A \neq \varnothing$, and $f, g: A \to \R$ are bounded functions. Let $m$ and $M$ denote the infimum and supremum respectively, then
				\begin{enumerate}[(i)]
		...
		\subsection{Integration on Higher Dimensions}
			...
			\begin{definition}
				...
				\begin{equation}
					\forall B_j \in \mc{B}(P), B_j' \in \mc{B}(P')\quad B_j' \subseteq B_j \lor (B_j^{' int} \cap B_j^{int} = \varnothing)
				\end{equation}
			\end{definition}
		...
		\subsection{Further Aspects}
			...
			\begin{theorem}[\red{Exchanging Differentiation and Integration}]
				...
				\begin{enumerate}[(i)]
					...
					\item and $\forall\ x_j \in \ve{x}$, $\pd{f(\ve{x}, \ve{t})}{x_j}$ is continuous (differentiable w.r.t \vex),
				\end{enumerate}
			...
			\subsubsection{Improper Integrals}
				...
				\begin{corollary}
					...
					\begin{equation}
						\exists\ \red{p > n},\ C > 0\ s.t.\ |f(\ve{x})| \leq \frac{1}{\norm{\ve{x}}^p}\ \forall \ve{x} \in \R^n
					\end{equation}
					then $\idotsint_{\R^n}f\ d\Omega$ is absolutely convergent.
				\end{corollary}
				}
	...
	\section{Vector Calculus}
		...
		\subsection{Green's Theorem}
			\subsubsection{Preliminary Definitions}
				...
				\begin{theorem}[Green's Theorem]
					Suppose $S \subseteq \R^2$ is a regular region with piecewise smooth boundary $\partial S$. Suppose $\ve{F}$ is a $C^1$ vector field defined on $\overline{S}$, then
					\begin{equation}
				...
				\begin{theorem}[Application: Moving the Curve]
					Suppose we wish to compute 
					\begin{align}
						\int_{C_1} \ve{F}\cdot d\ve{x}
					\end{align}
					for some piecewise smooth curve $C_1$. For any piecewise smooth curve $C_2$ such that $C_1 \cup C_2$ and $C_1 \cap C_2$ has zero content, such that $C_1 \cup C_2 = \partial S$ for some regular region $S \subset \R^2$.
					We can compute the desired line integral using 
					\begin{align}
						\iint_S \Big(\pd{F_2}{x_1} - \pd{F_1}{x_2}\Big)\ dA &= \int_{\partial S} \ve{F}\cdot d\ve{x} \\
						&= \int_{C_1 \cup C_2} \ve{F}\cdot d\ve{x} \\
						&= \int_{C_1} \ve{F}\cdot d\ve{x} + \int_{C_2} \ve{F}\cdot d\ve{x}
					\end{align}
				\end{theorem}
		...
		\subsection{Divergence Theorem}
			\begin{remark}
				vector field integral on boundary (2-dimensional sub-manifold) of region in $\R^3$ ($\ve{F} \cdot \ve{n}\ dA$ as 2-form) and scalar valued function ($\dive (\ve{F})\ dV$ as 3-form) in a region (3-dimensional sub-manifold).
			\end{remark}
			
			\begin{theorem}[Divergence Theorem]
				Let $R \subseteq \R^3$ be a \ul{regular region} with \ul{piecewise smooth boundary} $\partial S$. And $\ve{n}$ is the \ul{outer} normal vector on $\partial S$, then,
				\begin{equation}
			...
			\begin{theorem}[Application: Moving the Surface]
				Given a regular region $R \subset \R^3$ with piecewise smooth boundary $\partial R$ consisting of $S$ and $R'$. And let $\veF$ be a vector field defined on an open set containing $R$. We can find 
				\begin{equation}
					\iint_{S} \mathbf{F} \cdot \mathbf{n} d A
				\end{equation}
				by
				\begin{equation}
					\iiint_{R} \nabla \cdot \mathbf{F} d V=\iint_{\partial R} \mathbf{F} \cdot \mathbf{n} d A=\iint_{S} \mathbf{F} \cdot \mathbf{n} d A+\iint_{S^{\prime}} \mathbf{F} \cdot \mathbf{n} d A
				\end{equation}
				so
				\begin{equation}
					\iint_{S} \mathbf{F} \cdot \mathbf{n} d A=\iiint_{R} \nabla \cdot \mathbf{F} d V-\iint_{S^{\prime}} \mathbf{F} \cdot \mathbf{n} d A
				\end{equation}
			\end{theorem}
		...
		\subsubsection{The Orientation}
			...
			\begin{remark}[Informal Interpretation of Stoke's Orientation]
				The orientation for $\partial S$ may be described by saying if someone circles $\partial S$ with their head pointing in the direction of the unit normal $\ve{n}$, and with $S$ on the left, then the Stokes orientation corresponds to the direction in which they travel around $\partial S$.
			\end{remark}
		
		\subsubsection{Stokes Theorem in $\R^3$}
			\begin{theorem}[Stokes Theorem, Special Case]
				Assume that $S$ is a \ul{piecewise smooth} surface in $\R^3$ with boundary $\partial S$, that $S$ is oriented the unit normal $\ve{n}$ and that $\partial S$ has the compatible (Stoke) orientation ($\ve{n} \times \ve{t}$ points into $S$). Assume also that $\veF$ is any vector field that is $C^1$ in an open set containing $S$. Then
				\begin{equation}
					\iint_S \curl(\ve{F})\cdot \ve{n}\ dA = \int_{\partial S} \ve{F}\cdot d\ve{x}
				\end{equation}
			\end{theorem}
			...
			\begin{definition}
				A piecewise smooth surface $S \subset \R^3$ is \textbf{closed} if there exists subsets $S_1, S_2$ such that $\partial S_1 = \partial S_2 = S_1 \cap S_2$, also, $\partial S_1$ and $\partial S_2$ are piecewise smooth and have \ul{opposite orientations}, where $S_1, S_2$ have the same orientation as $S$.
			\end{definition}
			...
			\begin{theorem}[Application: Moving the Surface]
				Let $S$ and $S'$ be two oriented surfaces such that $\partial S = \partial S'$ (\emph{have the same orientation}), then 
				\begin{equation}
		...
		\subsubsection{The Generalization(Optional)}
...
%			\end{proposition}
		\subsection{Vector Fields that are Gradients or Curls}
			...
			\begin{corollary}
				If the domain of $\veG$ is a convex set containing the \ul{origin}, then we can define $\ve{p} = (a, b, c) = (0, 0, 0)$, and use a straight line $\veg(t):[0,1] \to \tx{Domain}(\veG) := (xt, yt, zt)$:
				\begin{equation}
		...
		\subsection{Summary}
			\subsubsection{Line Integrals}
				\begin{align}
					\tx{Arc length}\quad &\int_C 1\ d\vex =\int_a^b \norm{\veg'(t)}\ dt\\
					\tx{Integral of scalar valued function on curve }\quad &\int_C f(\vex)\ d\vex =\int_a^b f(\veg(t))\ \norm{\veg'(t)}\ dt\\
					\tx{Integral of vector field on curve }\quad &\int_C \veF(\vex)\cdot d\vex = \int_a^b \veF(\veg(t))\cdot \veg'(t)\ dt
				\end{align}
				
			\subsection{Surface Integrals}
				\begin{align}
					\tx{Surface area}\quad &\iint_S 1\ dA = \iint_W \norm{\partial_u \veG \times \partial_v \veG}\ du dv \\
					\tx{Scalar function on surface}\quad &\iint_S f(\vex)\ dA = \iint_W f(\veG(\ve{u})) \norm{\partial_u \veG \times \partial_v \veG}\ du dv \\
					\tx{Vector field on surface}\quad &\iint_S \veF(\vex)\cdot \ve{n}\ dA = \iint_W \veF(\veG(\ve{u}) \cdot \left(\partial_u \veG \times \partial_v \veG \right)\ du dv
				\end{align}
\end{document}
