		...
		\begin{assumption} Assumptions on linear regression on time series data:
			\begin{enumerate}[(i)]
				...
				\item \textbf{Homoscedasitcity}
					...
					\end{equation}
				\item \red{\textbf{No Serial Correlation}}
					\begin{equation}
...
%		\end{definition}
		...
		\begin{definition}
			...
			\begin{enumerate}[(i)]
				...
				\item \red{$Cov(Y_t, Y_s) = Cov(Y_{t+r}, Y_{s+r})\ \forall t,s,r\in \Z$.}
			\end{enumerate}
		\end{definition}
...
%		\end{definition}
	...
	\subsection{Loss Function}
		...
		\begin{example}[Symmetric Loss Functions]
			\begin{align}
				L(e) = a e^2,\ a > 0 \\
				L(e) = a\ \abs{e},\ a > 0
			\end{align}
		...
		\begin{example}[Asymmetric Loss Functions]
			\begin{align}
...
%					\var{Y_t} = \sigma_\varepsilon^2
		...
		\subsubsection{AP($p$) process}
			...
			\begin{theorem}
				\red{An autoregressive process is stationary \ul{if and only if} all roots of its characteristic polynomial are \hl{outside} the unit circle on $\C$.}
			\end{theorem}
