...
%		\end{figure*}
	...
	\section{Lecture 10 Nov. 29 2018}
		\subsection{Case 3}
			\begin{assumption}
				\begin{enumerate}
					...
					\item no depreciation, no growth
					\item infinite time horizon
					\item $H=1$
				\end{enumerate}
			\end{assumption}
			
			\paragraph{Controls} $c_t$ and $K_{t+1}$
			\paragraph{Budget} At each time period $t$,
			\begin{equation}
			...
			\end{gather}
			\begin{remark}
				Equation (3) implies $\lambda_t > 0,\quad \forall t \in \R_+$, so KTT complementary slackness condition suggests budget constraint (1) are satisfied with equality 
			\end{remark}
			
			\paragraph{Inter-Temporal Consumption}
				\begin{gather*}
					(3) \implies \lambda_t = e^{-\rho t}\frac{1}{c_t} \land \lambda_{t+1} = e^{-\rho (t+1)}\frac{1}{c_{t+1}}\\
					(5) \implies \lambda_t = \lambda_{t+1}(1+r_{t+1}) \\
					\iff \frac{1}{c_t} = (1+r_{\textcolor{red}{t+1}})e^{-\rho}\frac{1}{c_{t+1}}
				\end{gather*}
			\paragraph{Intra-Temporal Consumption-Leisure}
				\begin{gather*}
					(4) \implies e^{-\rho t} \frac{b}{1-\ell_t} = \lambda_t w_t \\
			...
			\paragraph{Inter-Temporal Leisure}
				\begin{gather*}
					(4) \land (5) \implies \frac{b}{1-\ell_t} \frac{1}{w_t} = e^{-\rho} (1+r_{t+1}) \frac{b}{1-\ell_{t+1}}\frac{1}{w_{t+1}} \\
		...
		\subsection{Case 4}
			...
			\begin{equation}
				\mc{L} = \expat{0}{\sum_{t=0}^\infty e^{-\rho t}[\ln(c_t) + b \ln(1-\ell_t)] + \lambda_t [w_t \ell_t + r_t k_t - c_t - K_{t+1} + K_t - G_t]}
			\end{equation}
			...
			\begin{remark}
				...
				\]
				\textbf{No} uncertainty presents in the shadow price $\lambda_t$ for any given time period $t$.
			\end{remark}
			...
			\begin{gather}
				...
				\implies \frac{1}{c_t} = e^{-\rho} \Big\{\expat{t}{\frac{1}{c_{t+1}}} \expat{t}{1 + r_{t+1}} + Cov_t(\frac{1}{c_{t+1}}, 1 + r_{t+1}) \Big\}
			\end{gather}
			
			\paragraph{Inter-Temporal Leisure}
			\begin{gather}
				e^{-\rho t} \frac{b}{(1-\ell_t) w_t} = e^{-\rho (t+1)} \expat{t}{\frac{b}{(1-\ell_{t+1})w_{t+1}}} \\
				\implies \frac{1}{1-\ell_t} \frac{1}{w_t} = e^{-\rho} \expat{t}{\frac{1}{1-\ell_{t+1}}\frac{1}{w_{t+1}}}
			\end{gather}
			
			\begin{proposition}
				If $r_{t+1}$ is high when $c_{t+1}$ is also high, i.e. \[
				Cov_t(1 + r_{t+1}, \frac{1}{c_{t+1}}) < 0
				\]
				then this would reduce the expected marginal benefit lower than a case where $r_{t+1}$ and $\frac{1}{c_{t+1}}$ are orthogonal.
				Let $\tilde{c}_t$ be current consumption when $Cov < 0$, and $\hat{c}_t$ denotes the current assumption when $Cov = 0$, then 
				\begin{gather*}
					\frac{1}{\tilde{c}_t} = \frac{1}{\hat{c}_t} + Cov_t(\frac{1}{c_{t+1}}, r_{t+1} + 1) e^{-\rho} \\ 
					\implies \frac{1}{\tilde{c}_t} < \frac{1}{\hat{c}_t}
				\end{gather*}
			\end{proposition}
\end{document}
