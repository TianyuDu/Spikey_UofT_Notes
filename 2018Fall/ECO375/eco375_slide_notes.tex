...
\begin{document}
	...
	\section{Slide 12: Specification and Data Problems}
		...
		\subsubsection{RESET: Nested Alternatives}
			...
			\]
			1) Run above regression, estimate $\hat{y}$. \\
			2) And add non-linear transformations of the \textbf{estimation} to construct an \textbf{expanded regression model}.
			\[
				y = \beta_0 + \beta_1 x_1 + \cdots + \beta_k x_k + \delta_1 \textcolor{red}{\hat{y}^2} + \delta_2 \textcolor{red}{\hat{y}^3} + u
			\]
			3) Use F-test to test the joint significance with $H_0: \delta_1 = \delta_2 = 0$. 
			...
			\end{example}
		\subsubsection{RESET: Non-nested Alternatives}
			...
			\end{gather}
			1) Expanded regression: 
			\[
				y = \beta_0 + \gamma_1 x_1 + \gamma_2 x_2 + \gamma_3 \log(x_1) + \gamma_4 \log(x_4) + u
			\]
			2-i) (F) test for specification (1): $H_0: \gamma_1 = \gamma_2 = 0$. \\
			2-ii) (F) test for specification (2): $H_0: \gamma_3 = \gamma_4 = 0$.
		\subsection{Davidson-MacKinnon test: non-nested alternatives}
			\par Let $\hat{y}_1$ and $\hat{y}_2$ denote the fitted values from (1) and (2) respectively. \\
			\newline
			1-i) Test for specification (1) with $H_0: \theta_1=0$. (If (1) is corrected, $\hat{y}_2$ should be insignificant.
			\begin{equation}
				y = \beta_0 + \beta_1 x_1 + \beta_2 x_2 + \theta_1 \hat{y}_{\textcolor{red}{2}} + u
			\end{equation}
			1-ii) Test for specification (2) with $H_0: \theta_1=0$:
			\begin{equation}
				y = \beta_0 + \beta_1 \log(x_1) + \beta_2 \log(x_2) + \theta_1 \hat{y}_{\textcolor{red}{1}} + u
			\end{equation}
			\begin{remark}
				\hl{In Davison-MacKinnon test, its possible for us to reject or accept both specifications.}
			\end{remark}
		\subsection{Proxy Variables}
			\subsubsection{Procedures}
				\par For the original model
				\begin{equation}
					y = \beta_0 + \beta_1 x_1 + \cdots + \beta_k x_k^{uob} + u
				\end{equation}
				where $x_k^{uob}$ is unobserved. \\
				\paragraph{Select} Choose an observed variable $x_k$ is a \textbf{proxy} for $x_k^{uob}$ such that
				\begin{equation}
					x_k^{uob} = \delta_0 + \delta_k x_k + v_3,\ \delta_k \neq 0
				\end{equation}
				\paragraph{Plug-in} plug (6) into (5)
				\begin{equation}
				...
				\begin{assumption}
					...
					\begin{enumerate}
						\item $u$ is uncorrelated with $x_1, x_2, \dots, x_k^{uob}, x_k$. 
						\item $v$ is uncorrelated with $x_1, x_2, \dots, x_k$.
						\[
							\expect{x_k^{uob}|x_1,x_2,\dots,x_k} = \expect{\delta_0 + \delta_k x_k + v|x_1,x_2,\dots,x_k} = \delta_0 + \delta_k x_k
						\]
					\end{enumerate}
				\end{assumption}
				\begin{remark}
					Under above assumptions, the OLS estimator for $(\beta_1, \beta_2, \dots \textcolor{red}{\beta_k \delta_k})$ is still \hl{consistent} and \hl{unbiased}.
				\end{remark}
			\subsubsection{Proxy Bias}
				If $x_k^{uob}$ is correlated with all $\{x_1, x_2, \dots, x_k\}$, i.e. 
				\[
					x_k^{uob} = \delta_0 + \delta_1 x_1 + \delta_2 x_2 + \dots + \delta_k x_k + v_k
				\]
				the for the coefficient of $x_j$ in the original regression, we are effective estimating 
				\[
					\beta_j + \beta_k \delta_j
				\]
		...
		\subsection{Measurement Error in an Explanatory Variable}
			...
			\[
				y = \beta_0 + \beta_1 x_1^{true} + u
			\]
			but we can only observe $x_1 = x_1^{true} + e_1$. Assuming \textbf{measurement error} satisfies
			\[
				\expect{e_1} = 0
			\]
			...
			\]
			\begin{assumption}
				$u$ is uncorrelated with both $x_1$ and $x_1^{true}$.
			\end{assumption}
			...
			\subsubsection{Case 2 $Cov(x_1^{true}, e_1) = 0$: Classical errors-in-variance(CEV)}
				\begin{gather*}
					Cov(x_1, e_1) = \expect{(x_1 - \overline{x}_1)(e_1 - \overline{e}_1)} \\
					= \expect{x_1 e_1} \\
					= \expect{(x_1^{true} + e_1) e_1} \\
					= \expect{x_1^{true}e_1 + e_1^2} \\
					= 0 + \expect{e_1^2} \\
					= \expect{(e_1 - \overline{e}_1)^2} \\
					= \sigma_{e_1}^2 \neq 0
				\end{gather*}
				Then the \hl{estimator $\hat{\beta}_1$  is biased and inconsistent}.

		\subsection{Measurement Error in Dependent Variable}
			\par Consider model 
			\[
				y^{true} = \textbf{X}\vec{\beta} + u
			\]
			and the actually observed $y$ is $y = y^{true} + e_0$. If we regress the observed $y$ on explanatory variables, we are effectively estimating
			\[
				y = \textbf{X}\vec{\beta} + \textcolor{red}{(u+e_0)}
			\]
			note that we would now have higher residual variance $\sigma^2 + \sigma_{e_0}^2$ and the variance for OLS estimator is inflated
			\[
				Var(\vec{\beta}) = (\sigma^2 + \sigma_{e_0}^2) (\textbf{X}'\textbf{X})^{-1}
			\]
			\hl{If $\expect{e_1|\textbf{X}}=0$ then $\hat{\beta}$ is still unbiased, otherwise in trouble.}
