...
\begin{document}
	...
	\section{Slide 14: Two Stage Least Square}
		...
		\begin{enumerate}
			\item \textbf{Stage 1} With excluded instrumental variables $\{z_i\}_{i \in \mc{I}}$. OLS Regress each endogenous $\{y_i\}_{i \in \mc{E}}$ on all instrumental variables and included exogenous variables. With reduced form equations,
			\[
				\hat{y}_j = \hat{\pi}_0 + \hat{\pi}_1 z_1 + \hat{\pi}_2 z_2 + 
				\dots + \hat{\pi}_n z_n
			\]
			...
			\item \textbf{Stage 2} Regress $y$ on $\{\hat{y}_j\}_{j \in \mc{I}}$ and all other included exogenous variables to obtain $\hat{\beta}_{i, 2SLS}$.
		\end{enumerate}		
