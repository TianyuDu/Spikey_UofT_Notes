...
%		\end{figure*}
	...
	\section{Lecture 10 Nov. 29 2018}
		...
		\subsection{Social Planner Problem}
			\par The social planner maximize household utility subject to aggregate resource constraint.
			\begin{assumption}
				For simplicity, $\delta=0$ and there's no growth, population is fixed at $N$.
			\end{assumption}
			\paragraph{Optimization}
			\begin{gather*}
				\max_{\{c_t, \ell_t, K_{t+1}\}_{t=0}^\infty} \expat{0}{\sum_{t=0}^\infty e^{-\rho t} [\ln(c_t) + b \ln(1-\ell_t)]N} \\
				s.t.\ 0 \leq K_t^\alpha (A_t \ell_t N)^{1-\alpha} - Nc_t - K_{t+1} + K_t - G_t
			\end{gather*}
			\paragraph{First Order Necessary Condition}
			\begin{gather*}
				\pd{\mc{L}}{c_t} = \expat{t}{e^{-\rho t} \frac{1}{c_t}N - \lambda_t N} = 0 \\
				\implies e^{-\rho t} \frac{1}{c_t} = \lambda_t
			\end{gather*}
			for all $t$ and all sates of the world.
			\begin{gather*}
				\pd{\mc{L}}{\ell_t} = \expat{t}{e^{-\rho t} (\frac{-b}{1-\ell_t})N + \lambda_t [(1-\alpha)K_t^\alpha (A_t N)^{1-\alpha} \ell_t ^ {-\alpha}]} = 0 \\
			...
			\par Note from the decentralized economy,
			\begin{gather*}
		...
		\begin{assumption} For simplicity, assume
			\begin{enumerate}
				\item $\delta = 1$
			\end{enumerate}
		\end{assumption}
