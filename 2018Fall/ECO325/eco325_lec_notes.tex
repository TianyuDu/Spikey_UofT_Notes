...
\begin{document}
	...
	\begin{itemize}
		...
		\end{equation}
		\item (\ul{Endogenous saving decisions}) A key difference from the Solow model is that the saving decision and the capital stock are determined by the interaction of maximizing households and firms.
	\end{itemize}
	...
	\begin{remark}
		$\rho$ measures consumers' attitude between present and future utility.
		...
		The greater $\rho$ is, the less value household puts on future consumption/utility (\emph{consumers are less patient}).
		Notice that $\rho$ cannot be negative in this model, since with $\rho < 0$ household values utility at $t=\infty$ infinitely more than current utility, the lifetime utility does not converge.
	...
	\end{remark}
	\begin{remark}
		The budget constraint implies that the \ul{present discounted value of lifetime consumption} must not exceed the \ul{present discounted value of labor income} plus \ul{initial wealth}.
	\end{remark}
	...
	\end{equation}
	\\
	Then normalize budget constraint (5) into per unit of effective labor variables to get an equivalent form.
	\begin{gather*}
		\int_{t=0}^\infty e^{-R(t)} C(t) \frac{L(t)}{H}\ dt  \leq \frac{K(0)}{H} + \int_{t=0}^\infty e^{-R(t)} w(t) \frac{A(t)L(t)}{H}\ dt \\
		\iff 
	...
	\end{gather*}
	\\
	The \textbf{assets} owned by household on date $t=s$ is given by
	\begin{gather*}
	...
	\section{Lecture 6 Oct 18. 2018}
		\subsection{Firm Setup}
			...
			\end{remark}
			\begin{assumption}
				Technology in this model, $A(t)$, is given exogenously so it isn't a choice for firms.
			\end{assumption}
		...
		\subsection{Firm's Optimization}
			...
			\end{gather*}
			\begin{remark}
				To show above result, substitute $Y$ using $AL f(\frac{K}{AL})$.
			\end{remark}
		...
		\par From the firms problem, $r(t) = f'(k(t))$, so in equilibrium, 
		\[
			\frac{\dot{c}(t)}{c(t)} = \frac{f'(k(t)) - \rho - \theta g}{\theta}
		\]
		In the long run, $\dot{c}(t) = 0$. This happens if and only if 
		\[
			f'(k^*) = \rho + \theta g
		\]
		Therefore 
...
%		\end{figure*}
		...
		\par For any given value of $k(0)$, there exists an \ul{unique} value of $c(0)$, so that one economy starts with $k(0), c(0)$ would evolve and converge to the steady state $E$. Such path of convergence is called \textbf{saddle path}.
		\begin{figure*}[h]
			\centering
