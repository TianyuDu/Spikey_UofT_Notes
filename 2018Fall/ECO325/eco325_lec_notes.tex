...
\begin{document}
	...
	\subsection{Speed of Convergence}
	    ...
	    So
	    \begin{equation}
	        \dot{k}(k) \approx (n + g + \delta) (\alpha (k^*) - 1) (k - k^*)
	    \end{equation}
	    Let $\lambda := (n + g + \delta) (1 - \alpha (k^*))$ then 
	    \begin{gather}
	        \dot{k}(k(t)) \approx -\lambda(k(t)-k^*) \\
	        \implies \textcolor{orange}{
	            k(t) - k^* \approx e^{-\lambda t}
	            }
	    \end{gather}
	    \begin{proof}[Derive]
	        Let $X(t) := k(t) + k^*$ \\
	        And $\dot{X}(t) = \dot{k}(t)$ \\
	        So we have an equation $\dot{X}(t) \approx -\lambda X(t)$
	    \end{proof}
	    Then note that 
	    \begin{gather*}
	        y(t) = f(k(t)) \\
	       ...
	       \implies y(t) - y^* \approx e^{- \lambda t} (y(0) - y^*)
	    \end{gather*}
	    ...
	    \begin{example}
	        ...
	        \begin{proof}
	            ...
	            \begin{gather*}
	                \frac{y(\hat{t} - y^*)}{y(0) - y^*} = 0.5 \approx e^{-\lambda t}\\
	                \implies \ln(0.5) \approx -\lambda \hat{t} \\
	                \implies \hat{t} = \frac{-\ln(0.5)}{0.04} \approx 17.33
	            \end{gather*}
	        \end{proof}
	    \end{example}
\end{document}
